...
<div class="max-w-5xl mx-auto" x-data="applicationWizard()">
    ...
    <div class="mb-8">
        <div class="flex items-center justify-between mb-2">
            <h1 class="text-2xl font-bold text-gray-900 dark:text-white">Dayhome Application</h1>
...
<form method="POST" action="{{ route('applicant.applications.store') }}" class="space-y-6" x-ref="applicationForm">
    @csrf
...
document.addEventListener('alpine:init', () => {
    Alpine.data('applicationWizard', () => ({
        ...
        formData: {
            educator_first_name: @json(old('educator_first_name', explode(' ', auth()->user()->name)[0] ?? '')),
            educator_last_name: @json(old('educator_last_name', explode(' ', auth()->user()->name)[1] ?? '')),
            email: @json(old('email', auth()->user()->email)),
            phone: @json(old('phone', auth()->user()->phone ?? '')),
            address_line_1: @json(old('address_line_1')),
            city: @json(old('city', auth()->user()->city ?? '')),
            province: @json(old('province', auth()->user()->province ?? 'Alberta')),
