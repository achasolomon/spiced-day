...
<div class="max-w-4xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-4">
        <a href="{{ route($isAdmin ? 'admin.applications.show' : 'consultant.applications.show', $application ?? $appointment->application ?? request('application_id')) }}" 
        ...
        <div>
            ...
            <p class="text-gray-600 dark:text-gray-400 mt-1">{{ $checklist->name }}</p>
        </div>
    </div>

    <form action="{{ route($isAdmin ? 'admin.inspections.store' : 'consultant.inspections.store') }}" method="POST" enctype="multipart/form-data" x-data="inspectionForm()">
        @csrf
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" value="{{ ucwords(str_replace('_', ' ', $inspectionType)) }}" 
                ...
                <div>
                    ...
                    <input type="datetime-local" name="conducted_at" value="{{ now()->format('Y-m-d\TH:i') }}" 
            ...
            <div class="mt-6 p-4 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg border border-yellow-200 dark:border-yellow-800">
                ...
                <p class="text-sm text-yellow-800 dark:text-yellow-400">
                    <strong>If any item fails:</strong> The inspection will be marked as failed, and you'll be asked to make a decision on how to proceed (continue to next stage, schedule follow-up, or reject application).
                </p>
            </div>
        </div>
                    ...
                    <div class="p-4 bg-gray-50 dark:bg-gray-900/50 rounded-lg" 
                        ...
                        <div class="flex items-center gap-4">
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="radio" 
                                       ...
                                       value="pass" 
                                       @click="updateProgress(); itemStatus = 'pass'; showNotes = false"    
                            ...
                            <label class="flex items-center gap-2 cursor-pointer">
                                <input type="radio" 
                                       name="checklist_results[{{ $item->code }}][status]" 
                                       value="n/a" 
                                       @click="updateProgress(); itemStatus = 'n/a'; showNotes = false"
                        ...
                        <div x-show="showNotes" x-transition class="mt-3">
                            <textarea name="checklist_results[{{ $item->code }}][notes]" 
                                      ...
                                      class="w-full px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-orange-500 dark:bg-gray-700 dark:text-white"
                                      :class="{ 'border-red-500 dark:border-red-500': itemStatus === 'fail' }"></textarea>
        ...
        <div class="flex items-center justify-between mt-6">
            ...
            <div class="flex gap-3">
                <button type="submit" name="save_as_draft" value="1" @click="saveDraft()"
