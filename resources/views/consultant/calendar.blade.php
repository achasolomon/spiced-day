...
<div class="space-y-4 md:space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <button 
            ...
            class="w-full sm:w-auto px-4 md:px-6 py-2.5 md:py-3 bg-orange-600 hover:bg-orange-700 text-white rounded-lg font-medium transition-colors text-sm md:text-base">
    ...
    <div x-show="showModal" 
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20">
            ...
            <div class="relative bg-white dark:bg-gray-800 rounded-lg md:rounded-xl shadow-xl max-w-2xl w-full p-4 md:p-6">
                <div class="flex items-center justify-between mb-4 md:mb-6">
                    <h3 class="text-xl md:text-2xl font-bold text-gray-900 dark:text-white">Schedule New Appointment</h3>
                    ...
                    <div class="space-y-3 md:space-y-4">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
                            <div>
                                ...
                                <select name="application_id" required class="w-full px-3 md:px-4 py-2 text-sm md:text-base border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-orange-500 dark:bg-gray-700 dark:text-white">
                                    <option value="">Select application...</option>
                                    @foreach(auth()->user()->assignedApplications()->active()->get() as $app)
                                        <option value="{{ $app->id }}">{{ $app->full_name }} - {{ $app->application_number }}</option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="title" required placeholder="e.g., Initial Home Inspection" class="w-full px-3 md:px-4 py-2 text-sm md:text-base border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-orange-500 dark:bg-gray-700 dark:text-white">
                        </div>

                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Date & Time*</label>
                                <input type="datetime-local" name="scheduled_at" required class="w-full px-3 md:px-4 py-2 text-sm md:text-base border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-orange-500 dark:bg-gray-700 dark:text-white">
                            </div>

                            <div>
                                <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Duration (minutes)*</label>
                                <input type="number" name="duration" required value="120" min="30" max="480" class="w-full px-3 md:px-4 py-2 text-sm md:text-base border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-orange-500 dark:bg-gray-700 dark:text-white">
                            </div>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Location*</label>
                            <input type="text" name="location_address" required placeholder="Address" class="w-full px-3 md:px-4 py-2 text-sm md:text-base border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-orange-500 dark:bg-gray-700 dark:text-white">
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Notes</label>
                            <textarea name="description" rows="3" placeholder="Additional notes..." class="w-full px-3 md:px-4 py-2 text-sm md:text-base border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-orange-500 dark:bg-gray-700 dark:text-white"></textarea>
                        </div>

                        <input type="hidden" name="consultant_id" value="{{ auth()->id() }}">
                        <input type="hidden" name="location_type" value="home">
                    </div>
                    
                    <div class="flex flex-col sm:flex-row items-center gap-3 mt-4 md:mt-6">
                        <button type="button" @click="showModal = false" class="w-full sm:flex-1 px-4 py-2 text-sm md:text-base bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-lg hover:bg-gray-300 dark:hover:bg-gray-600 transition-colors">
                            Cancel
                        </button>
                        <button type="submit" class="w-full sm:flex-1 px-4 py-2 text-sm md:text-base bg-orange-600 hover:bg-orange-700 text-white rounded-lg font-medium transition-colors">
                            Schedule Appointment
                        </button>
                    </div>
                </form>
