<div x-data="editItemModal()" 
    ...
    <div class="flex min-h-full items-center justify-center p-4">
        <div x-show="open" 
            ...
            <!-- Modal Body -->
            <form method="POST" :action="formAction" enctype="multipart/form-data" @submit="submitForm">
                @csrf
                @method('PUT')
                ...
                <div class="px-6 py-6 space-y-6 max-h-[70vh] overflow-y-auto">
                    ...
                    <div x-show="formData.type === 'document'">
                        ...
                        <div x-show="formData.file_path" class="mb-3 p-3 bg-blue-50 dark:bg-blue-900/30 rounded-lg">
                            <div class="flex items-center justify-between">
                                <div class="flex items-center gap-2">
                                    <svg class="w-5 h-5 text-blue-600 dark:text-blue-400" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd" d="M4 4a2 2 0 012-2h4.586A2 2 0 0112 2.586L15.414 6A2 2 0 0116 7.414V16a2 2 0 01-2 2H6a2 2 0 01-2-2V4z" clip-rule="evenodd"></path>
                                    </svg>
                                    <span class="text-sm font-medium text-blue-700 dark:text-blue-300">Current file attached</span>
                                </div>
                                <a :href="formData.download_url" 
                                   target="_blank"
                                   class="text-xs text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 underline">
                                    View
                                </a>
                            </div>
                        </div>
                        ...
                        <label for="edit_file" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                            <span x-show="formData.file_path">Replace Document</span>
                            <span x-show="!formData.file_path">Upload Document</span>
                            <span x-show="!formData.file_path" class="text-red-500">*</span>
                        </label>
                        <input type="file" 
                               id="edit_file" 
                               name="file" 
                        ...
                        <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
                            <span x-show="formData.file_path">Leave empty to keep current file. </span>
                    ...
                    <div x-show="formData.type === 'date'">
                        <label for="edit_date_value" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                            Date <span class="text-red-500">*</span>
                        </label>
                        <input type="date" 
                               id="edit_date_value" 
                               name="date_value" 
                               x-model="formData.date_value"
                    ...
                    <div x-show="formData.type === 'boolean'">
                        <label class="flex items-center gap-3 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg cursor-pointer">
                            <input type="checkbox" 
                                   name="boolean_value" 
                                   value="1"
                                   x-model="formData.boolean_value"
                    ...
                    <div>
                        <label for="edit_expiry_date" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                            Expiry Date (Optional)
                        </label>
                        <input type="date" 
                               id="edit_expiry_date" 
    ...
    return {
        ...
        openModal(item) {
            this.formData = {
                ...
                boolean_value: item.boolean_value == 1,
                expiry_date: item.expiry_date || '',
                notes: item.notes || '',
                file_path: item.file_path || '',
                download_url: item.download_url || ''
            };
            this.formAction = `/applicant/profile/items/${item.id}`;
            this.open = true;
        },
