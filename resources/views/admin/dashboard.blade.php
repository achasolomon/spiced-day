...
<div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-gray-600 dark:text-gray-400 mt-1">Monitor and manage your application system</p>
        </div>
    ...
    @if(count($alerts) > 0)
        <div class="space-y-3">
            @foreach($alerts as $alert)
                <div class="bg-{{ $alert['type'] }}-50 dark:bg-{{ $alert['type'] }}-900/20 border border-{{ $alert['type'] }}-200 dark:border-{{ $alert['type'] }}-800 rounded-xl p-4">
                    <div class="flex items-start gap-3">
                        ...
                        <div class="flex-1">
                            <p class="font-semibold text-gray-900 dark:text-white">{{ $alert['message'] }}</p>
                            @if(isset($alert['action']))
                                <a href="{{ $alert['action'] }}" class="text-sm text-{{ $alert['type'] }}-600 hover:text-{{ $alert['type'] }}-700 font-medium mt-1 inline-block">
                                    View Details →
                                </a>
                            @endif
                        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div class="bg-gradient-to-br from-purple-500 to-purple-600 rounded-2xl p-6 text-white shadow-lg">
            <div class="flex items-center justify-between mb-4">
                ...
                <div class="text-right">
                    ...
                    <p class="text-2xl font-bold">+{{ $stats['new_applications_today'] }}</p>
                </div>
            </div>
            <div>
                <p class="text-purple-100 text-sm">Total Applications</p>
                <p class="text-4xl font-bold mt-1">{{ number_format($stats['total_applications']) }}</p>
            </div>
        </div>
        ...
        <div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-2xl p-6 text-white shadow-lg">
            ...
            <div>
                <p class="text-blue-100 text-sm">Active Applications</p>
                <p class="text-4xl font-bold mt-1">{{ number_format($stats['active_applications']) }}</p>
            </div>
        </div>
        ...
        <div class="bg-gradient-to-br from-green-500 to-green-600 rounded-2xl p-6 text-white shadow-lg">
            <div class="flex items-center justify-between mb-4">
                ...
                <div class="text-right">
                    ...
                    <p class="text-2xl font-bold">{{ number_format(($stats['approved_this_month'] / max($stats['total_applications'], 1)) * 100, 1) }}%</p>
                </div>
            </div>
            <div>
                <p class="text-green-100 text-sm">Approved This Month</p>
                <p class="text-4xl font-bold mt-1">{{ number_format($stats['approved_this_month']) }}</p>
            </div>
        </div>
        ...
        <div class="bg-gradient-to-br from-orange-500 to-orange-600 rounded-2xl p-6 text-white shadow-lg">
            <div class="flex items-center justify-between mb-4">
                ...
                <div class="text-right">
                    <p class="text-orange-100 text-sm">Consultants</p>
                    <p class="text-2xl font-bold">{{ $stats['active_consultants'] }}</p>
                </div>
            </div>
            <div>
                <p class="text-orange-100 text-sm">Total Users</p>
                <p class="text-4xl font-bold mt-1">{{ number_format($stats['total_users']) }}</p>
            </div>
        </div>
    </div>
        ...
        <div class="lg:col-span-2">
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
                <div class="px-6 py-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between">
                    ...
                    <a href="{{ route('admin.applications.index') }}" class="text-sm text-purple-600 hover:text-purple-700 font-medium">View All →</a>
                </div>
                <div class="p-6">
                    <div class="space-y-4">
                            ...
                            <div class="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-900/50 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-900 transition-colors">
                                <div class="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-sm text-gray-600 dark:text-gray-400">{{ $app->application_number }}</p>
                                    </div>
                                </div>
                                <div class="flex items-center gap-3">
                                    ...
                                    <a href="{{ route('admin.applications.show', $app) }}" class="px-3 py-1.5 bg-purple-600 hover:bg-purple-700 text-white rounded-lg text-sm font-medium transition-colors">
                                        View
                                    </a>
                                </div>
                            </div>
                        @empty
                            <p class="text-center text-gray-500 dark:text-gray-400 py-8">No recent applications</p>
                        @endforelse
                    </div>
                </div>
            </div>
                    ...
                    @foreach($statusDistribution as $status => $count)
                        <div>
                            <div class="flex items-center justify-between mb-1">
                                <span class="text-sm text-gray-600 dark:text-gray-400">{{ ucwords(str_replace('_', ' ', $status)) }}</span>
